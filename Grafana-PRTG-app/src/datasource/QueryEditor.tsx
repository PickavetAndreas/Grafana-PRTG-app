...
import { LegacyForms, Select, PanelOptionsGroup, InlineFormLabel } from '@grafana/ui';
import { QueryEditorProps, SelectableValue, DataSourceApi } from '@grafana/data';
import { DataSource, getItems } from './DataSource';
import {
  ...
  return (
    <section>
      <SelectRender
        ...
        changeFunc={updateQuery}
        currentValue={defaultedQuery.queryMethod}
        isMulti={false}
      ></SelectRender>
      <PanelOptionsGroup>
        <PrtgQueryEditorHelper updateQuery={updateQuery} query={defaultedQuery} jsonData={Props.datasource.jsonData} />
      </PanelOptionsGroup>
    </section>
  );
}

const PrtgQueryEditorHelper = ({ updateQuery, query, jsonData }: any) => {
  const RenderAllGroupsSelect: any = (
    <SelectRender
      queryKey={'selectedGroup'}
      selectOptions={getItemsFromDataSource('groups', '0', ['name', 'objid'], jsonData)}
      changeFunc={updateQuery}
      currentValue={query.selectedGroup}
      isMulti={false}
    ></SelectRender>
  );
    ...
    case 'table':
      return (
        <section style={{ display: 'flex', flexDirection: 'row' }}>
          ...
          <SelectRender
            queryKey={'tableOption'}
            selectOptions={tableOptions}
            changeFunc={updateQuery}
            currentValue={query.tableOption}
            isMulti={false}
          ></SelectRender>
          ...
          <SelectRender
            queryKey={'tableColumnItems'}
            selectOptions={getTableColumnOptions(query)}
            changeFunc={updateQuery}
            currentValue={query.tableColumnItems}
            isMulti={true}
          ></SelectRender>
          <FormField
            tooltip={'only possible to filter on given columns'}
            value={query.tableFilterString}
            label={'Filter'}
            onChange={e => updateQuery('tableFilterString', e.target.value)}
          ></FormField>
        </section>
      );
    case 'historicdata':
      return (
        <section style={{ display: 'flex', flexDirection: 'row' }}>
          ...
          <SelectRender
            ...
            selectOptions={getItemsFromDataSource('devices', query.selectedGroup.value, ['name', 'objid'], jsonData)}
            changeFunc={updateQuery}
            currentValue={query.selectedDevice}
            isMulti={false}
          ></SelectRender>
          <InlineFormLabel width={'auto'}>{'Sensor:'}</InlineFormLabel>
          <SelectRender
            queryKey={'selectedSensor'}
            selectOptions={getItemsFromDataSource('sensors', query.selectedDevice.value, ['name', 'objid'], jsonData)}
            changeFunc={updateQuery}
            currentValue={query.selectedSensor}
            isMulti={false}
          ></SelectRender>
        </section>
      );
      ...
      return (
        <section style={{ display: 'flex', flexDirection: 'row' }}>
          <FormField
            value={query.rawURI}
            label={'URI: '}
            onChange={e => updateQuery('rawURI', e.target.value)}
          ></FormField>
          <FormField
            value={query.rawQuerytext}
            label={'Query text: '}
            onChange={e => updateQuery('rawQuerytext', e.target.value)}
          ></FormField>
        </section>
      );
    case 'status':
      return <section>{'no edit options for status request'}</section>;
    default:
  ...
  if (query.tableOption.value === 'message') {
    return (
      <section>
        ...
        <SelectRender
          queryKey={'messageFilterDrel'}
          selectOptions={messageFilterDrelList}
          changeFunc={updateQuery}
          currentValue={query.messageFilterDrel}
          isMulti={false}
        ></SelectRender>
      </section>
    );
  ...
  return (
    <div>
      <Select
        ...
        onChange={e => (changeFunc(queryKey, e), setSelected(e))}
        value={selected}
      />
    </div>
  );
...
const getItemsFromDataSource = (contentInput: string, idInput: string, columnsInput: string[], jsonData:any): SelectableValue[] => {
  const selects: SelectableValue[] = getGrafVars();
  getItems({ content: contentInput, id: idInput, columns: columnsInput },jsonData).then((apiRes: any) => {
